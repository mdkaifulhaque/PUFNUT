// snacks.js - background-image loader + add-to-cart + fallback handling

const STORAGE_KEY = "PUFNUT_PRODUCTS";
let snackProducts = [];

(function loadSnackProducts(){
  const saved = localStorage.getItem(STORAGE_KEY);
  const allProducts = saved ? JSON.parse(saved) : [];
  snackProducts = allProducts.filter(p => p.category === 'snacks');
  renderHeroCards();
})();
...
function addToCart(pid){ const prod = snackProducts.find(x=>x.id===pid); if(!prod) return {ok:false}; const cart = readCart(); const ex = cart.find(i=>i.id===pid); if(ex) ex.qty=(ex.qty||1)+1; else cart.push({id:prod.id,name:prod.name,price:prod.price,qty:1,img:prod.img}); saveCart(cart); return {ok:true}; }
...
function renderHeroCards(){
  ...
  if (snackProducts.length === 0) {
    container.innerHTML = "<p style='color:#fff;text-align:center;padding:20px;'>No snacks available at the moment.</p>";
    return;
  }
